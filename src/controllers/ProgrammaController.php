...
class ProgrammaController{
    ...
    static function getProgramma($req, $res, $service, $app){
        ...
        $stm = $app->db->prepare('SELECT programma.*, atleta.nome, atleta.cognome FROM programma INNER JOIN atleta ON programma.id_atleta=atleta.id_atleta WHERE programma.deleted=false AND programma.id_coach=:id_coach ORDER BY programma.data_inizio DESC');
